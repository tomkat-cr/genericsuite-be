"""
DbAbstractorDynamodb: Database abstraction layer for DynamoDb
"""
...
DEFAULT_WRITE_CAPACITY_UNITS = int(
    os.environ.get("AWS_DYNAMODB_DEFAULT_WRITE_CAPACITY_UNITS", "1")
)
DEFAULT_READ_CAPACITY_UNITS = int(
    os.environ.get("AWS_DYNAMODB_DEFAULT_READ_CAPACITY_UNITS", "1")
)
    ...
    def __next__(self):
        ...
        if isinstance(self._data_set, dict):
            if self._num > 0:
                raise StopIteration
            self._num += 1
            return self.remove_decimal_types(self.id_addition(self._data_set))
        if (
            (not self._limit or self._num <= self._limit)
            and self._data_set
            and self._num < len(self._data_set)
        ):
...
class DynamoDbTableAbstract(DynamoDbUtilities):
    ...
    def get_table_definitions(self):
        ...
        self._global_secondary_indexes = item_structure.get(
        ...
        if not isinstance(value, dict):
            ...
            if "$in" in value or "$nin" in value:
                ...
                conditions.append(
                    {
                        "value": values,
                        "function": ("NOT " if "$nin" in value else "")
    ...
    def get_condition_expresion_values(self, data_list, separator=" AND "):
        ...
        for item in data_list:
            _ = DEBUG and log_debug(
                "|||---> get_condition_expresion_values" + f" | item: {item}"
            )
        ...
        separator = (
            " OR "
            if "$or" in separator
            else " AND " if "$and" in separator else separator
        )
    ...
    def update_one(self, key_set, update_set_original):
        ...
        result = None
        if (
            "$set" in update_set_original
            or "$addToSet" in update_set_original
            or "$pull" in update_set_original
        ):
            try:
    ...
    def count_documents(self, filter):
        ...
        return self.generic_query(
            query_params=filter, query_type="find", select="COUNT"
        )
    ...
    def create_tables(self, dynamodb_table_structures: dict):
        ...
        for item_name in item_list:
            ...
            table = self._db.create_table(
                ...
                AttributeDefinitions=item_props["AttributeDefinitions"],
    ...
    def list_collections(self, collection_name: str = None,
        ...
        try:
            ...
            table_names.extend(
                [
                    tn
                    for tn in response.get("TableNames", [])
